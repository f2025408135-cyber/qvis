import re
with open("tests/test_database_abstraction.py", "r") as f:
    content = f.read()

# Fix indentation. The indent should be 4 spaces.
content = content.replace("    await db.save_threat(threat)", "    await db.save_threat(threat)")

# Ah the replacement block had an extra indent maybe? Let's just write the whole test properly
with open("tests/test_database_abstraction.py", "w") as f:
    f.write("""\"\"\"Tests for the database abstraction layer.\"\"\"
